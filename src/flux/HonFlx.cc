...
HonFlx::HonFlx() : FlxTable::FlxTable()
{
  hfluxLoaded=false;
  lowfluxLoaded=false;
...
double HonFlx::GetFlux(float E,NEUTRINO::FLAVOR nuFlav,Time_Struct t)
{
  ...
  if(honEFull[0]>E or honEFull[140]<E)
    {
  ...
  double BMin=log(double(honTableEn[iE-1][nuFlav][0]/honTableEn[iE][nuFlav][0]))/log(double(honEFull[iE-1]/honEFull[iE]));
  double AMin=honTableEn[iE-1][nuFlav][0]/pow(honEFull[iE-1],BMin);

  double BMax=log(double(honTableEn[iE-1][nuFlav][1]/honTableEn[iE][nuFlav][1]))/log(double(honEFull[iE-1]/honEFull[iE]));
  double AMax=honTableEn[iE-1][nuFlav][1]/pow(honEFull[iE-1],BMax);
...
double HonFlx::GetFlux(float phi, float cosZ,float E,NEUTRINO::FLAVOR nuFlav,Time_Struct t)
{
 ...
 phi=v.Phi();*/
 //phi in tables is pointing to source of neutrino, we want pointing away from source
...
double HonFlx::GetFlux(float cosZ,float E,NEUTRINO::FLAVOR nuFlav)
{
  //for E<.1 GeV where we only have 1D info.  Uses same technique as 3D get flux but w/o phi
  ...
  if((honEL[0]>E) or (honEL[34]<E))
    {
      fprintf(stderr,"honEL[0] %f honEL[34] %f honEL[17] %f\n",honEL[0],honEL[34],honEL[17]);
      fprintf(stderr,"Error: Energy out of range low E angle ver E=%f\n",E);
      return -1;
    }
  int iCosZL,iEL;//index lower entry (below value)
  int iCosZH,iEH;//index uper entry (above value)
  float CosZL;
  float CosZH;
  if(cosZ<-0.95)
    {
  ...
  float fluxELCosZL=honTableL[iEL][iCosZL][nuFlav][0]*(1-solarAct)+honTableL[iEL][iCosZL][nuFlav][1]*solarAct;
  float fluxELCosZH=honTableL[iEL][iCosZH][nuFlav][0]*(1-solarAct)+honTableL[iEL][iCosZH][nuFlav][1]*solarAct;
  ...
  float fluxEHCosZL=honTableL[iEH][iCosZL][nuFlav][0]*(1-solarAct)+honTableL[iEH][iCosZL][nuFlav][1]*solarAct;
  float fluxEHCosZH=honTableL[iEH][iCosZH][nuFlav][0]*(1-solarAct)+honTableL[iEH][iCosZH][nuFlav][1]*solarAct;
  ...
  double B=log(double(fluxEL/fluxEH))/log(double(honEL[iEH-1]/honEL[iEH]));
  double A=fluxEL/pow(honEL[iEH-1],B);
  ...
  for(int iS=0;iS<2;iS++)
    {
       for(int iCosZ=0;iCosZ<20;iCosZ++)
	 {
	   ...
	   for(int iPhi=0;iPhi<12;iPhi++)
	     {
	       for(int iE=101;iE<107;iE++)
		 {
		   honE[iE]=VolkE[iE-99];
		   honTable[iE][iCosZ][iPhi][NEUTRINO::MU][iS]=VolkFlux[iE-99][iCosZV]*VolkCoeff[NEUTRINO::MU]/pow(honE[iE],3);
		   honTable[iE][iCosZ][iPhi][NEUTRINO::MU_BAR][iS]=VolkFluxBar[iE-99][iCosZV]*VolkCoeff[NEUTRINO::MU_BAR]/pow(honE[iE],3);
		   honTable[iE][iCosZ][iPhi][NEUTRINO::E][iS]=VolkFlux[iE-99][iCosZV]*VolkCoeff[NEUTRINO::E_BAR]/pow(honE[iE],3);;
		   honTable[iE][iCosZ][iPhi][NEUTRINO::E_BAR][iS]=VolkFluxBar[iE-99][iCosZV]*VolkCoeff[NEUTRINO::E_BAR]/pow(honE[iE],3);;
		 }
	     }
...
int HonFlx::LoadLowFluxTable(std::ifstream& fileStrm)
{

  int iCosZ=0;
  char line[200];

  float honLNorm[2][4]={{1.00,1.05,1.00,1.00},{0.89,0.95,0.89,0.89}};
  for(int iZ=0;iZ<20;iZ++)
      ...
      if(iZ>0)
	{
      ...
      for(int iE=0;iE<72;iE++)
	{
	  if(iE>34)
	    {
	      //don't load values above .1Gev
	      fileStrm.getline(line,200);
	      continue;
	    }
	  fileStrm>>honEL[iE];
	  ...
	  float flxTmpE,flxTmpMu,flxTmpEBar,flxTmpMuBar,dummy;
	  for(int i=0;i<4;i++)
	    {
	      fileStrm>>dummy;
	    }
	  ...
	  for(int iS=0;iS<2;iS++)
	    {
	      ...
	      honTableL[iE][iCosZ][NEUTRINO::E][iS]=flxTmpE*honLNorm[iS][NEUTRINO::E];
	      honTableL[iE][iCosZ][NEUTRINO::MU][iS]=flxTmpMu*honLNorm[iS][NEUTRINO::MU];
	      honTableL[iE][iCosZ][NEUTRINO::E_BAR][iS]=flxTmpEBar*honLNorm[iS][NEUTRINO::E_BAR];
	      honTableL[iE][iCosZ][NEUTRINO::MU_BAR][iS]=flxTmpMuBar*honLNorm[iS][NEUTRINO::MU_BAR];
	  ...
	  //clear out blank space at end of line
	  fileStrm.getline(line,200);
	}
    }
 ...
 void HonFlx::FillIntegratedTables()
 {
   if(!(lowfluxLoaded and hfluxLoaded))
     {
	 ...
	 for(int nuFlav=0;nuFlav<4;nuFlav++)
	   {
	     for(int iE=0;iE<141;iE++)
	       {
		 honTableEn[iE][nuFlav][iS]=0;
		 if(iE<34)
		   {
		     honEFull[iE]=honEL[iE];
		     for(int iCosZ=0;iCosZ<20;iCosZ++)
		       {
			 honTableEn[iE][nuFlav][iS]=honTableEn[iE][nuFlav][iS]+honTableL[iE][iCosZ][nuFlav][iS];
		       }
		     honTableEn[iE][nuFlav][iS]=honTableEn[iE][nuFlav][iS]*0.1*2.*Pi;
		     
		   }
		 else
		   {

		     honEFull[iE]=honE[iE-34];
		     for(int iCosZ=0;iCosZ<20;iCosZ++)
		       {
			 for(int iPhi=0;iPhi<12;iPhi++)
			   {

			     honTableEn[iE][nuFlav][iS]=honTableEn[iE][nuFlav][iS]+honTable[iE-34][iCosZ][iPhi][nuFlav][iS];
